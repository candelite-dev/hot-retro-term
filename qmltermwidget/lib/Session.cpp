...
#ifdef Q_OS_WIN
    ptySlaveFd = -1;   // Windows has no slave fd. getPtySlaveFd() is only used by
                       // qtermwidget.cpp, which is not part of the QML build.
#else
...
void Session::onReceiveBlock( const char * buf, int len )
{
    ...
    // Latin-1 mangles multi-byte UTF-8, so the payload is only safe to use
    // as a "data arrived" pulse (hot-retro-term uses it to hide the boot
    // overlay), never as text.
