...
void Emulation::receiveData(const char* text, int length)
{
     ...
     */
    // Decode straight from the incoming buffer and pair surrogates inline,
    // avoiding the QByteArray and std::wstring intermediate copies that were
    // previously made for every PTY block.
    const QString str = _toUtf16(QByteArrayView(text, length));

    //send characters to terminal emulator
    for (qsizetype i = 0; i < str.length(); ++i)
    {
        const QChar ch = str.at(i);
        if (ch.isHighSurrogate() && i + 1 < str.length() && str.at(i + 1).isLowSurrogate())
        {
            receiveChar(static_cast<wchar_t>(QChar::surrogateToUcs4(ch, str.at(i + 1))));
            ++i;
        }
        else
        {
            receiveChar(static_cast<wchar_t>(ch.unicode()));
        }
    }
