...
private:
    ...
    // Image-space rect covering all cells with RE_BLINK, refreshed by
    // updateImage(); lets blinkEvent() repaint only the blinking area.
    QRect _blinkImageRect;

    // Per-font glyph advance cache; cleared in fontChange().
    int cachedCharWidth(char32_t c);
    QHash<char32_t, int> _charWidthCache;
