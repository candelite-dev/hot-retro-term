...
,_mouseAutohideDelay(-1)
{
  ...
  connect(urlFilter, &UrlFilter::activated, this, [this](const QUrl& url, bool) {
...
// NOTE: unused since the QML port — see the comment in updateImage() for
// why blitting cannot work on an FBO-backed QQuickPaintedItem.
...
void TerminalDisplay::updateImage()
{
  ...
  // Scroll-blit (upstream's scrollImage() call) stays disabled by design:
  // it relied on widget-level bitblt, and an FBO-backed QQuickPaintedItem
  // has no API to shift already-rendered FBO content — update() repaints,
  // it never copies. Scroll floods are already coalesced to ~25 repaints/s
  // by Emulation's bulk timers (BULK_TIMEOUT1/2), which caps the cost.
  // Alternatives evaluated 2026-07: scene-graph content-offset scrolling
  // (rewrites this class's rendering contract) and an Image render target
  // with QImage::scroll (full texture re-upload per frame — a regression at
  // retina sizes). Neither is worth it.
  ...
  for (y = 0; y < linesToUpdate; ++y)
  {
    ...
    for (x = 0; x < columnsToUpdate; ++x)
    {
      if ((newLine[x].rendition & RE_BLINK) != 0) {
        _hasBlinker = true;
        blinkImageRect |= QRect(x, y, 1, 1);
      }

      // The dirty rect below always spans the full line width, so any
      // changed cell marks the whole line for repaint.
      if (!updateLine && newLine[x] != currentLine[x])
        updateLine = true;
    }
  ...
  _blinkImageRect = blinkImageRect;
...
int TerminalDisplay::cachedCharWidth(char32_t c)
{
    auto it = _charWidthCache.constFind(c);
    if (it != _charWidthCache.constEnd())
        return it.value();

    // Advance lookups go through font shaping — cache per code point.
    // The cache is cleared on every font change (fontChange()).
    const QFontMetrics fm(font());
    const int width = (c > 0xFFFF)
        ? fm.horizontalAdvance(QString::fromUcs4(&c, 1))
        : fm.horizontalAdvance(QChar(static_cast<ushort>(c)));
    _charWidthCache.insert(c, width);
    return width;
}

void TerminalDisplay::paint(QPainter *painter)
{
    // Prime the terminal font once; drawCharacters only switches fonts when
    // a fragment's bold/italic/underline attributes actually differ.
    painter->setFont(font());
...
void TerminalDisplay::paintFilters(QPainter& painter)
{
    ...
    while (iter.hasNext())
    {
        ...
        for ( int line = spot->startLine() ; line <= spot->endLine() ; line++ )
        {
            ...
            while ( QChar::isSpace(_image[loc(endColumn,line)].character) && endColumn > 0 )
                endColumn--;
            ...
            r.setCoords( startColumn*_fontWidth + 1 + leftMargin,
                         ...
                         (line+1)*_fontHeight - 1 + _topBaseMargin );
            // Invert colors for link hotspots (always visible, CRT aesthetic)
            if ( spot->type() == Filter::HotSpot::Link )
            {
                for (int col = startColumn; col < endColumn; col++) {
                    int idx = loc(col, line);
                    if (idx < 0 || idx >= _imageSize) continue;

                    QColor fg = _image[idx].foregroundColor.color(colorTable());
                    QColor bg = _image[idx].backgroundColor.color(colorTable());

                    QRect charRect(col * _fontWidth + leftMargin,
                                  line * _fontHeight + _topBaseMargin,
                                  _fontWidth, _fontHeight);

                    painter.fillRect(charRect, fg);
                    wchar_t ch = _image[idx].character;
                    if (ch > 0x20) {
                        painter.setPen(bg);
                        painter.setFont(font());
                        QString cs = ch > 0xFFFF
                            ? QString::fromUcs4(reinterpret_cast<const char32_t*>(&ch), 1)
                            : QString(QChar(static_cast<uint>(ch)));
                        painter.drawText(charRect, Qt::AlignLeft | Qt::AlignVCenter, cs);
                    }
                }
            }
...
void TerminalDisplay::drawContents(QPainter &paint, const QRect &rect)
{
  ...
  for (int y = luy; y <= rly; y++)
  {
    ...
    for (; x <= rlx; x++)
    {
      ...
      int charWidth = cachedCharWidth(static_cast<char32_t>(c));
      ...
      while (x+len <= rlx &&
             ...
             !smallWidth &&
             !(_fixedFont && (nxtC = _image[loc(x+len,y)].character) && (nxtCharWidth = cachedCharWidth(static_cast<char32_t>(nxtC))) < _fontWidth) &&
             !bigWidth &&
...
void TerminalDisplay::blinkEvent()
{
  ...
  if (_blinkImageRect.isValid())
    update(QRegion(imageToWidget(_blinkImageRect)));
  else
    update();
}
...
//            m_session->removeView(this);
        ...
        // hot-retro-term: exposing receivedData
        connect(m_session, SIGNAL(receivedData(QString)), this, SIGNAL(receivedData(QString)));
